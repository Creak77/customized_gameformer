...
class MapEncoder(nn.Module):
    def __init__(self, tesk_length):
        super(MapEncoder, self).__init__()
        # traffic light and stop sign location
        self.traffic_light = nn.Embedding(4, 64, padding_idx=0)
        self.stpo_sign = nn.Embedding(2, 64, padding_idx=0)
        self.waypoint = nn.Linear(3, 64)
        self.task = nn.Linear(tesk_length, 64, padding_idx=0)
    ...
    def forward(self, inputs):
        traffic_light = self.traffic_light(inputs[0].int())
        stop_sign = self.stpo_sign(inputs[1].int())
        waypoint = self.waypoint(inputs[2])
        task = self.task(inputs[3].int())

        image_info = self.image_encoder(inputs[4])

        lane_embedding = torch.cat([traffic_light, stop_sign, waypoint, task, image_info], dim=-1)
        output = self.position_encode(self.pointnet(lane_embedding))
