...
class Encoder(nn.Module):
    def __init__(self, neighbors_to_predict, layers=6):
        ...
        #
        # need to change those encoder to have map encoder and traffic light encoder
    ...
    def forward(self, inputs):
        ...
        encoded_map_lanes = self.map_encoder(len(task))
        ...
        for i in range(N):
            map_info, map_mask = self.segment_map(task, encoded_map_lanes)
            fusion_input = torch.cat([encoded_actors, map_info], dim=1)
